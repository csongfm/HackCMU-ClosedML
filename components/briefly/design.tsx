...
export function FlowShell({ children }: { children: ReactNode }) {
  return <MotionConfig reducedMotion="user" transition={{ type: 'spring', stiffness: 170, damping: 25 }}><main className="flow-shell"><FlowPaths />{children}<footer className="flow-footer"><span className="wordmark">briefly<span aria-hidden="true">&reg;</span></span><span>A LITTLE NEWS. A LOT MORE YOU.</span><a href="#top" aria-label="Back to top"><ArrowUpRight /></a></footer></main></MotionConfig>;
}
export function Brand() { return <Link href="/" className="wordmark" aria-label="Briefly home">briefly<span aria-hidden="true">&reg;</span></Link>; }
